...
(1)~A pre-registered $120$-run design at two scales (TinyLlama-1.1B-Chat and LLaMA-3.2-3B) that separates partition seeds from training seeds under a frozen federation and LoRA configuration. (2)~A method-of-moments variance decomposition with cluster-bootstrap intervals, showing on TinyLlama that partition and interaction shares dominate the residual ($0.508$ and $0.469$ versus $0.023$), while on LLaMA-3.2-3B the interaction dominates ($0.764$) and the partition main-effect interval includes zero. (3)~A ranking-stability analysis conditioned on effect size: the near-tied FedIT--FLoRA pair flips under few draws, whereas gaps of about $0.023$ to $0.031$ never reverse in a single draw. (4)~A draws-needed table for pre-registered deltas and for the observed gaps, including $206$ and $46$ paired draws for the near-tie at the two scales. We make no method recommendation.

Section~\ref{sec:related} situates the work; Section~\ref{sec:setup} states the design and variance model; Section~\ref{sec:results} reports the measurements; Section~\ref{sec:discussion} gives reporting recommendations and limitations.
